# contest_tools/contest_specific_annotations/wrtc_2018_scoring.py
#
# Purpose: Provides contest-specific scoring logic for the WRTC 2018 contest.
#
...
def calculate_points(df: pd.DataFrame, my_call_info: Dict[str, Any]) -> pd.Series:
    """
    Calculates QSO points for an entire DataFrame based on WRTC 2018 rules.
    ...
    - Note: Same rules as WRTC-2026 (uniform points for both CW and SSB).
    """
    my_continent = my_call_info.get('Continent')
    if not my_continent:
        raise ValueError("Logger's own Continent must be provided for WRTC scoring.")

    # Enforce the Europe-only rule for WRTC 2018
    if my_continent != 'EU':
        logging.warning("WRTC 2018 scoring is only valid for European stations. This log is from a non-EU station and will result in a score of 0.")
