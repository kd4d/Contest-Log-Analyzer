...
## [0.28.2-Beta] - 2025-08-04
### Fixed
# - Corrected the subplot creation logic in `_create_pie_chart_subplot` to
#   properly handle different grid layouts, fixing a TypeError.
## [0.28.1-Beta] - 2025-08-02
### Added
# - Added the `_create_cumulative_rate_plot` shared helper function to
#   centralize the logic for generating QSO and Point rate plots.
# - Added the `_create_pie_chart_subplot` shared helper function to
#   centralize the logic for generating pie charts.
import pandas as pd
...
def _create_pie_chart_subplot(fig, gs, df, title, radius, is_not_to_scale=False):
    ...
    """
    # Create a nested GridSpec for the pie and its table
...
def _create_cumulative_rate_plot(logs, output_path, band_filter, metric_name, value_column, agg_func, report_id):
    ...
    for log in logs:
        ...
        # Reindex to ensure the plot covers the full contest timeline
    ...
    # --- Inset Summary Table ---
    if summary_data:
        col_labels = ["Call", f"Total\n{metric_name}", "Run", "S&P", "Unk"]
