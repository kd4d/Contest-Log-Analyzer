...
[cite_start]The Contest Log Analyzer is a powerful command-line tool designed for amateur radio contesters who want to perform deep, data-driven analysis of their operating performance. [cite: 824] [cite_start]It goes beyond the simple score summary provided by most logging software, allowing you to: [cite: 825]

* [cite_start]Process raw Cabrillo log files into a clean, standardized format. [cite: 826]
* [cite_start]Automatically classify every QSO as "Run," "Search & Pounce," or "Unknown" to analyze your operating strategy. [cite: 827]
* [cite_start]Generate detailed reports and charts that compare your log against one or more others. [cite: 828]
* [cite_start]Analyze performance on a band-by-band basis to identify strengths and weaknesses. [cite: 829]
* [cite_start]Calculate contest-specific QSO points for supported contests. [cite: 830]
[cite_start]The ultimate goal of this program is to help you understand your contest operation in minute detail, identify missed opportunities, and improve your strategy for the next event. [cite: 830]
---
...
* [cite_start]**Your Cabrillo Log File(s)**: These are the standard log files generated by your contest logging software (e.g., `kd4d.log`, `n0ni.log`). [cite: 831] [cite_start]You can analyze a single log or compare multiple logs at once. [cite: 832]
* [cite_start]**Data Files**: The program requires specific data files to be placed in a central `data/` directory. [cite: 833]
    * [cite_start]`cty.dat`: Required for all contests. [cite: 834]
    * [cite_start]`arrl_10_mults.dat`: Required for the ARRL 10 Meter contest. [cite: 835]
    * [cite_start]`ARRLDXmults.dat`: Required for the ARRL DX contest. [cite: 836]
    * [cite_start]`NAQPmults.dat`: Required for NAQP and CQ 160-Meter contests. [cite: 837]
    * `SweepstakesSections.dat`: Required for ARRL Sweepstakes and ARRL Field Day.
* [cite_start]**An Environment Variable**: You must tell the program where to find your data files by setting an environment variable named `CONTEST_LOGS_REPORTS`. [cite: 839] [cite_start]This variable should point to the root directory that contains your `Logs`, `data`, and `reports` subdirectories. [cite: 840]
---

## 3. How to Run the Analyzer

[cite_start]The program is run from your command prompt or terminal using `main_cli.py`. [cite: 841]
#### **Basic Syntax**
...
* [cite_start]`--report <ReportID|all|chart|text|plot|animation>`: (Required) Specifies which report to generate. [cite: 842] [cite_start]Use a specific `ReportID` (e.g., `score_report`), `all` to generate every available report, or a category keyword like `chart` to run all chart reports. [cite: 843]
* [cite_start]`<LogFile1> ...`: (Required) One or more paths to the Cabrillo log files you want to analyze. [cite: 844]
* `--verbose`: (Optional) Enables detailed `INFO`-level status messages for debugging.
* [cite_start]`--include-dupes`: (Optional) By default, duplicate QSOs are ignored. [cite: 845] [cite_start]Use this flag to include them in all calculations. [cite: 846]
* [cite_start]`--mult-name <name>`: (Optional) For reports that analyze multipliers (like `missed_multipliers`), this specifies which multiplier to use (e.g., 'Countries', 'Zones'). [cite: 847]
* [cite_start]`--metric <qsos|points>`: (Optional) For the `cumulative_difference_plots` report, this specifies whether to compare QSO counts or Point totals. [cite: 848] Defaults to 'qsos'.
* [cite_start]`--debug-data`: (Optional) When used with a visual report (chart, plot, animation), this saves the report's source data to a `.txt` file in a `Debug/` subdirectory. [cite: 849]
...
[cite_start]The analyzer uses the `CONTEST:` field in your Cabrillo file header to automatically apply the correct rules. [cite: 850] The following contests are currently supported:
...
[cite_start]Use the `Report ID` with the `--report` command-line option. [cite: 851]
