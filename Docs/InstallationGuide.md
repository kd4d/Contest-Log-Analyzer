...
If your site looks broken (missing styles, JavaScript not working), collect static files:
...
#### If You See Database Migration Warnings

If you see errors like:
```
You have unapplied migration(s). Your project may not work properly 
...
#### Understanding Docker Exec Commands

**Why `docker compose exec`?** These commands run **inside the container** where Python and Django are installed. You cannot run Django management commands from your host system unless Python is installed there (which is not required for Docker deployment).
