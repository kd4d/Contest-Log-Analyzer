...
### Changed
# - Documented the new hybrid report model (`is_specialized` flag
#   and `included_reports` JSON key).
...
| `excluded_reports`| A list of generic `report_id` strings to disable for this contest (opt-out). | `[ "point_rate_plots" ]` |
| `included_reports`| *Optional.* A list of specialized `report_id` strings to explicitly enable for this contest (opt-in). | `[ "text_wae_score_report" ]` |
