...
def multiplier_dashboard(request, session_id):
    ...
    # Find combo_id from animation file
    anim_art = next((a for a in artifacts if a['report_id'] == 'interactive_animation'), None)
    if not anim_art:
        raise Http404("Analysis data invalid")
        
    combo_id = os.path.basename(anim_art['path']).replace('interactive_animation_', '').replace('.html', '')
    ...
    json_art = next((a for a in artifacts if a['report_id'] == 'json_multiplier_breakdown'), None)
    ...
    txt_bd_art = next((a for a in artifacts if a['report_id'] == 'text_multiplier_breakdown'), None)
    if txt_bd_art:
        breakdown_txt_rel_path = f"{report_rel_path}/{txt_bd_art['path']}"

    # 5. Discover HTML Version of Breakdown
    breakdown_html_rel_path = None
    html_bd_art = next((a for a in artifacts if a['report_id'] == 'html_multiplier_breakdown'), None)
    if html_bd_art:
